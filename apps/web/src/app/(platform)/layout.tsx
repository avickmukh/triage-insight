...
import { LayoutDashboard, Building2, CreditCard, Tag, ToggleLeft, Activity, ScrollText, Shield, ChevronRight, Trash2, Users } from 'lucide-react';
...
export default function PlatformAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const displayName = [(me as any)?.firstName, (me as any)?.lastName].filter(Boolean).join(' ') || (me as any)?.name || 'Admin';
    ...
    <div className="flex h-screen bg-gray-950 text-gray-100">
      <aside className="w-60 flex-shrink-0 border-r border-gray-800 flex flex-col">
        ...
        <div className="px-4 py-3 border-t border-gray-800 text-xs text-gray-500">
          <p className="font-medium text-gray-400">{displayName}</p>
          <p className="truncate">{(me as any)?.email}</p>
          <p className="text-violet-400/70 mt-0.5">{role}</p>
        </div>
      </aside>
