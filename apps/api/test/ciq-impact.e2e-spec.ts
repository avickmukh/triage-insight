...
const MOCK_THEME = {
  ...
  signalBreakdown: {
    ...
    sentimentPenalty: {
      ...
      weight: 0.15,
      contribution: 8.5,
      label: 'Sentiment Penalty',
    },
  },
  aggregatedPriorityScore: 78.5,
  aiSummary: 'Checkout slowness affects 12 enterprise customers.',
  aiExplanation:
    'This is a critical revenue path. Delays here directly cause cart abandonment.',
  aiRecommendation:
    'Optimise the payment gateway integration and add CDN caching.',
  aiConfidence: 0.87,
...
const MOCK_ROADMAP_ITEM = {
  ...
  description:
    'Checkout slowness affects 12 enterprise customers.\n\nWhy it matters: This is a critical revenue path.',
  status: 'EXPLORING',
  priorityScore: 78.5,
...
const mockPrisma = {
  ...
  workspaceMember: {
    findFirst: jest
      .fn()
      .mockResolvedValue({ role: 'ADMIN', userId: 'user-001' }),
  },
  theme: {
  ...
  roadmapItem: {
    ...
    findUnique: jest.fn().mockResolvedValue({
      ...MOCK_ROADMAP_ITEM,
      theme: { ...MOCK_ROADMAP_ITEM.theme, feedbacks: [] },
    }),
    count: jest.fn().mockResolvedValue(1),
  },
  customerSignal: {
...
describe('CIQ Impact API (e2e)', () => {
  ...
  describe('GET /api/workspaces/:workspaceId/themes/:id/ciq', () => {
    ...
    it('should return scoreExplanation with contribution values', async () => {
      ...
      const firstFactor = Object.values(explanation)[0] as {
        contribution: number;
        label: string;
      };
