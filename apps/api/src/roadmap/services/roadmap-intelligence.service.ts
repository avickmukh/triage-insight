...
export class RoadmapIntelligenceService {
  ...
  async getAiSuggestions(
    ...
    for (const theme of themes) {
      ...
      // --- CIQ score (0–100) ---
      const ciqScore = clamp((theme.ciqScore ?? theme.priorityScore ?? 0) * 100);
      ...
      const sourceScore = clamp(
        countNorm(supportCount, 20) * 0.40 +
        countNorm(feedbackCount, 30) * 0.30 +
        countNorm(voiceCount, 10) * 0.20 +
        countNorm(surveyCount, 20) * 0.10,
      );

      // --- Recency signal (0–100) ---
      const daysSinceScored = daysSince(theme.lastScoredAt);
      const recencyScore = clamp(
        daysSinceScored <= 7  ? 100 :
        daysSinceScored <= 14 ? 80  :
        daysSinceScored <= 30 ? 60  :
        daysSinceScored <= 60 ? 30  : 10,
      );
      ...
      const resurfacingBonus = prevWeek === 0 && currWeek > 0
        ? 80
      ...
      const activeSources = [feedbackCount, voiceCount, supportCount, surveyCount]
        .filter(c => c > 0).length;
      ...
      // --- Confidence ---
      const totalSignals = theme.totalSignalCount ?? (feedbackCount + voiceCount + supportCount + surveyCount);
      const confidence   = this.computeConfidence(totalSignals, activeSources, ciqScore);

      // --- Dominant driver ---
      const dominantDriver = this.computeDominantDriver({
        ciqScore, velocityScore, sentimentIntensity, sourceScore, recencyScore, resurfacingBonus,
      });
      ...
      const reason = this.buildReason(
        suggestionType,
        dominantDriver,
        { ciqScore, roadmapPriorityScore, velocityDelta, supportCount, feedbackCount, voiceCount, surveyCount, activeSources, avgSentiment },
      );
      ...
      const breakdown_raw = theme.signalBreakdown as Record<string, { value?: number; label?: string }> | null;
      const priorityReason = breakdown_raw
        ? this.buildPriorityReason(roadmapPriorityScore, dominantDriver, breakdown_raw)
        : null;
      ...
      suggestions.push({
        ...
        signalSummary: {
          ...
          activeSources,
          velocityDelta,
          sentimentScore: sentiments.length > 0 ? parseFloat(avgSentiment.toFixed(3)) : null,
        },
        breakdown: {
          ciqScore:             parseFloat(ciqScore.toFixed(1)),
    ...
    // ── 4. Summary counts ────────────────────────────────────────────────────
    const summary = {
      addToRoadmap:     sorted.filter(s => s.suggestionType === 'ADD_TO_ROADMAP').length,
      increasePriority: sorted.filter(s => s.suggestionType === 'INCREASE_PRIORITY').length,
      decreasePriority: sorted.filter(s => s.suggestionType === 'DECREASE_PRIORITY').length,
  ...
  private computeSuggestionType(
    ...
    if (velocityDelta !== null && velocityDelta > 30 && !isCommitted) return 'INCREASE_PRIORITY';
    ...
    switch (type) {
      case 'ADD_TO_ROADMAP':
        return (
          ...
          (ctx.supportCount > 0 ? ` ${ctx.supportCount} support ticket${ctx.supportCount !== 1 ? 's' : ''} are linked.` : '') +
          velStr + srcStr + sentStr +
      ...
      case 'INCREASE_PRIORITY':
        return (
          ...
          velStr + srcStr + sentStr +
          (ctx.feedbackCount > 0 ? ` ${ctx.feedbackCount} feedback item${ctx.feedbackCount !== 1 ? 's' : ''} linked.` : '') +
          ` Dominant driver: ${dominantDriver ?? 'CIQ Score'}.`
        );
  ...
  private buildConfidenceExplanation(
    ...
    const parts: string[] = [];
    if (ctx.feedbackCount > 0) parts.push(`${ctx.feedbackCount} feedback item${ctx.feedbackCount !== 1 ? 's' : ''}`);
    if (ctx.voiceCount > 0)    parts.push(`${ctx.voiceCount} voice signal${ctx.voiceCount !== 1 ? 's' : ''}`);
    if (ctx.supportCount > 0)  parts.push(`${ctx.supportCount} support ticket${ctx.supportCount !== 1 ? 's' : ''}`);
    if (ctx.surveyCount > 0)   parts.push(`${ctx.surveyCount} survey response${ctx.surveyCount !== 1 ? 's' : ''}`);

    const signalStr = parts.length > 0 ? `based on ${parts.join(', ')}` : 'based on limited signals';
  ...
  private buildPriorityReason(
    ...
    const topFactor = dominantDriver ?? Object.keys(breakdown)[0] ?? 'signal volume';
