...
describe('CiqService', () => {
  ...
  describe('scoreTheme — score range', () => {
    ...
    it('should return a priorityScore in the 0–100 range for high-signal input', async () => {
      ...
      const prisma = buildMockPrisma(feedbacks, [{ strength: 10 }], [], [], {}, clusters);
      const service = await createService(prisma);
  ...
  describe('scoreTheme — adaptive countNorm (small workspace)', () => {
    ...
    it('should rank a theme with 20 signals higher than one with 5 signals', async () => {
      const few  = Array.from({ length: 5 },  (_, i) => makeFeedback({ customerId: `c-${i}` }));
      const many = Array.from({ length: 20 }, (_, i) => makeFeedback({ customerId: `c-${i}` }));
  ...
  describe('scoreTheme — support signals', () => {
    it('should apply spike bonus when a cluster has hasActiveSpike=true', async () => {
      ...
      const prismaSpike   = buildMockPrisma(feedbacks, [], [], [], {}, clustersWithSpike);
      const prismaNoSpike = buildMockPrisma(feedbacks, [], [], [], {}, clustersWithoutSpike);
  ...
  describe('scoreTheme — severity scoring', () => {
    it('should score critical-keyword feedback higher than feature-request feedback', async () => {
      ...
      const prismaCritical = buildMockPrisma(criticalFeedbacks, [], [], [], {}, []);
      const prismaFeature  = buildMockPrisma(featureRequestFeedbacks, [], [], [], {}, []);

      const serviceCritical = await createService(prismaCritical);
      const serviceFeature  = await createService(prismaFeature);

      const resultCritical = await serviceCritical.scoreTheme('ws-test', 'theme-critical');
      const resultFeature  = await serviceFeature.scoreTheme('ws-test', 'theme-feature');

      expect(resultCritical.priorityScore).toBeGreaterThan(resultFeature.priorityScore);
    });
  });
  ...
  describe('scoreTheme — confidence score', () => {
    ...
    it('should give higher confidence when voice and support signals are present', async () => {
      ...
      const prismaRich  = buildMockPrisma([...feedbacks, ...voiceFeedbacks], [{ strength: 5 }], [], [], {}, clusters);
  ...
  describe('scoreTheme — score explanation', () => {
    it('should include all 5-factor explanation keys', async () => {
      ...
      const requiredKeys = ['volume', 'severity', 'frequency', 'friction', 'recency'];
    ...
    it('should have 5-factor weights that sum to 1.0', async () => {
      ...
      const BASE_KEYS = ['volume', 'severity', 'frequency', 'friction', 'recency'];
      const weightSum = BASE_KEYS.reduce((sum, k) => sum + (result.scoreExplanation[k]?.weight ?? 0), 0);
      expect(weightSum).toBeCloseTo(1.0, 2);
    });

    it('should include crmMultiplier in explanation when CRM data is present', async () => {
      const feedbacks = [
        makeFeedback({ customerId: 'c1', arrValue: 300000, accountPriority: AccountPriority.CRITICAL }),
      ];
      const prisma = buildMockPrisma(feedbacks, [], [], [], {}, []);
      const service = await createService(prisma);
    ...
    it('should NOT include crmMultiplier when no CRM data is present', async () => {
      const feedbacks = [
        makeFeedback({ customerId: 'c1', arrValue: 0 }),
      ];
      const prisma = buildMockPrisma(feedbacks, [], [], [], {}, []);
      const service = await createService(prisma);
  ...
  describe('scoreTheme — score ceiling', () => {
    it('should not inflate scores above 100 even with maximum signals', async () => {
      ...
      const deals = [
        { deal: { annualValue: 5000000, stage: DealStage.NEGOTIATION, status: DealStatus.OPEN } },
      ];
      const prisma = buildMockPrisma(feedbacks, [{ strength: 10 }], deals, [], {}, clusters);
      const service = await createService(prisma);
      const result = await service.scoreTheme('ws-test', 'theme-max');
      expect(result.priorityScore).toBeLessThanOrEqual(100);
    });
