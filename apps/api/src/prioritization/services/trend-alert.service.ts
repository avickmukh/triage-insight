  ...
  async getAlerts(workspaceId: string): Promise<TrendAlertResponse> {
    ...
    for (const t of themes) {
      ...
      if (delta >= VELOCITY_SPIKE_THRESHOLD) {
        const urgency: UrgencyLevel = delta >= VELOCITY_CRITICAL_THRESHOLD ? 'CRITICAL' : 'HIGH';
      ...
      if ((t.resurfaceCount ?? 0) > 0 && t.resurfacedAt) {
        ...
        if (ageDays <= RESURFACE_WINDOW_DAYS) {
          ...
          const urgency: UrgencyLevel = count >= 3 ? 'CRITICAL' : count >= 2 ? 'HIGH' : 'MEDIUM';
          const reason = [
          ...
          rawAlerts.push({
            ...
            _score: count * 30 + (urgency === 'CRITICAL' ? 50 : urgency === 'HIGH' ? 25 : 0),
          });
        }
      }
      ...
      if (dist) {
        ...
        if (total >= 5) {  // need at least 5 signals to be statistically meaningful
