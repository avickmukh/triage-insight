...
export class AggregationService {
  ...
  async getFeaturePriorityRanking(
    workspaceId: string,
    limit = 50,
  ): Promise<FeaturePriorityItem[]> {
    ...
    const ranked: FeaturePriorityItem[] = feedbacks.map((fb) => {
      ...
      const tagMult = strategicTag
        ? (STRATEGIC_TAG_MULTIPLIER[strategicTag] ?? 1.0)
        : 1.0;
      const dealValue = (firstTheme?.dealLinks ?? []).reduce((sum, dl) => {
        if (!dl.deal || dl.deal.status === DealStatus.LOST) return sum;
        return (
          sum +
          (dl.deal.annualValue ?? 0) * (DEAL_STAGE_WEIGHT[dl.deal.stage] ?? 0)
        );
      }, 0);
      ...
      const demandFactors = {
        ...
        sentimentPolarity:
          (fb.sentiment ?? 0) < 0 ? Math.abs(fb.sentiment ?? 0) * 100 : 0,
        surveyValidation: 0,
      };
      const demandNorm = clamp100(
        demandFactors.clusterSize * 0.35 +
          demandFactors.voteVelocity * 0.3 +
      ...
      const revenueNorm = clamp100(
        revenueFactors.arrWeighted * 0.4 +
          revenueFactors.dealBlocking * 0.3 +
          revenueFactors.enterpriseWeight * 0.2 +
          revenueFactors.churnRiskLinkage * 0.1,
      );

      const strategicFactors = {
        adminStrategicTag:
          strategicTag === 'strategic'
            ? 100
            : strategicTag === 'core'
              ? 70
              : strategicTag === 'nice-to-have'
                ? 30
                : 50,
      ...
      const strategicNorm = clamp100(
        (strategicFactors.adminStrategicTag * 0.5 +
          strategicFactors.roadmapAlignment * 0.35 +
          strategicFactors.productVision * 0.15) *
          tagMult,
      );
      ...
      const urgencyNorm = clamp100(
        urgencyFactors.negativeVoiceTrend * 0.4 +
          urgencyFactors.escalationFlags * 0.2,
      );
      ...
      const breakdown: PrioritizationScoreBreakdown = {
        demandStrength: {
          raw: demandNorm,
          normalised: demandNorm,
          weight: weights.demandStrengthWeight,
          contribution: demandNorm * weights.demandStrengthWeight,
          label: 'Demand Strength',
          factors: demandFactors,
        },
        revenueImpact: {
          raw: revenueNorm,
          normalised: revenueNorm,
          weight: weights.revenueImpactWeight,
          contribution: revenueNorm * weights.revenueImpactWeight,
          label: 'Revenue Impact',
          factors: revenueFactors,
        },
        strategicImportance: {
          raw: strategicNorm,
          normalised: strategicNorm,
          weight: weights.strategicImportanceWeight,
          contribution: strategicNorm * weights.strategicImportanceWeight,
          label: 'Strategic Importance',
          factors: strategicFactors,
        },
        urgencySignal: {
          raw: urgencyNorm,
          normalised: urgencyNorm,
          weight: weights.urgencySignalWeight,
          contribution: urgencyNorm * weights.urgencySignalWeight,
  ...
  async getThemePriorityRanking(
    workspaceId: string,
    limit = 50,
  ): Promise<ThemePriorityItem[]> {
      ...
      this.prisma.theme.findMany({
        ...
        select: {
          ...
          dealLinks: {
            select: {
              deal: {
                select: { annualValue: true, stage: true, status: true },
              },
            },
          },
          roadmapItems: {
    ...
    for (const spike of supportSpikes) {
      spikeByCluster.set(
        spike.clusterId,
        (spikeByCluster.get(spike.clusterId) ?? 0) + spike.zScore,
      );
    }
    ...
    const ranked: ThemePriorityItem[] = themes.map((theme) => {
      ...
      if (theme.manualOverrideScore != null) {
        const tagMult = theme.strategicTag
          ? (STRATEGIC_TAG_MULTIPLIER[theme.strategicTag] ?? 1.0)
          : 1.0;
        ...
        const placeholder: PrioritizationScoreBreakdown = {
          ...
          strategicImportance: {
            raw: overrideScore,
            normalised: overrideScore,
            weight: 1,
            contribution: overrideScore,
            label: 'Manual Override',
            factors: { manualOverride: theme.manualOverrideScore },
          },
          urgencySignal: {
            raw: 0,
            normalised: 0,
        ...
        return {
          ...
          revenueScore: theme.revenueScore ?? 0,
          ...
          uniqueCustomerCount: new Set(
            theme.feedbacks.map((f) => f.feedback.customerId).filter(Boolean),
          ).size,
          revenueInfluence: theme.revenueInfluence ?? 0,
          dealInfluenceValue: 0,
          strategicTag: theme.strategicTag,
          manualOverrideScore: theme.manualOverrideScore,
      ...
      const uniqueCustomerIds = new Set(
        feedbacks.map((f) => f.customerId).filter(Boolean),
      );
      const totalArr = feedbacks.reduce(
        (s, f) => s + (f.customer?.arrValue ?? 0),
        0,
      );
      const maxPriority = feedbacks.reduce(
        (max, f) =>
          Math.max(
            max,
            ACCOUNT_PRIORITY_MAP[
              f.customer?.accountPriority ?? AccountPriority.MEDIUM
            ],
          ),
        1,
      );
      const avgSentiment =
        feedbacks.length > 0
          ? feedbacks.reduce((s, f) => s + (f.sentiment ?? 0), 0) /
            feedbacks.length
          : 0;
      const avgVoteVelocity =
        feedbacks.length > 0
          ? feedbacks.reduce((s, f) => s + (f.voteVelocity ?? 0), 0) /
            feedbacks.length
          : 0;
      const avgUrgency =
        feedbacks.length > 0
          ? feedbacks.reduce((s, f) => s + (f.urgencySignal ?? 0), 0) /
            feedbacks.length
          : 0;
      const maxChurnRisk = feedbacks.reduce(
        (max, f) => Math.max(max, f.customer?.churnRisk ?? 0),
        0,
      );
      ...
      const spikeScore = theme.correlations.reduce(
        (sum, c) => sum + (spikeByCluster.get(c.id) ?? 0),
        0,
      );
      const tagMult = theme.strategicTag
        ? (STRATEGIC_TAG_MULTIPLIER[theme.strategicTag] ?? 1.0)
      ...
      const roadmapMult = roadmapStatus
        ? (ROADMAP_STATUS_MULTIPLIER[roadmapStatus] ?? 1.0)
        : 1.0;

      const demandNorm = clamp100(
        countNorm(feedbacks.length, 50) * 0.35 +
          countNorm(avgVoteVelocity, 5) * 0.3 +
          (avgSentiment < 0 ? Math.abs(avgSentiment) * 100 : 0) * 0.25,
      );
      const revenueNorm = clamp100(
        logNorm(totalArr, 8) * 0.4 +
          logNorm(dealInfluenceValue, 7) * 0.3 +
          (maxPriority / 4) * 100 * 0.2 +
          maxChurnRisk * 100 * 0.1,
      );
      const strategicNorm = clamp100(
        ((theme.strategicTag === 'strategic'
          ? 100
          : theme.strategicTag === 'core'
            ? 70
            : theme.strategicTag === 'nice-to-have'
              ? 30
              : 50) *
          0.5 +
      ...
      const urgencyNorm = clamp100(
        countNorm(spikeScore, 10) * 0.4 +
          avgUrgency * 100 * 0.4 +
          (maxChurnRisk >= 0.7 ? 80 : maxChurnRisk >= 0.4 ? 40 : 0) * 0.2,
      );
      ...
      const breakdown: PrioritizationScoreBreakdown = {
        ...
        revenueImpact: {
          ...
          contribution: revenueNorm * weights.revenueImpactWeight,
          label: 'Revenue Impact',
          factors: { totalArr, dealInfluenceValue, maxChurnRisk },
        },
        strategicImportance: {
          raw: strategicNorm,
          normalised: strategicNorm,
        ...
        urgencySignal: {
          ...
          contribution: urgencyNorm * weights.urgencySignalWeight,
          label: 'Urgency Signals',
          factors: { spikeScore, avgUrgency, maxChurnRisk },
        },
      };
  ...
  async getRoadmapRecommendations(
    workspaceId: string,
    limit = 30,
  ): Promise<RoadmapRecommendationItem[]> {
    ...
    if (roadmapItems.length === 0) {
      ...
      return topThemes.map((theme) => {
        const score = clamp100(
          theme.ciqScore ?? (theme.priorityScore ?? 0) * 100,
        );
        ...
        const breakdown: PrioritizationScoreBreakdown = {
          demandStrength: {
            ...
            normalised: demandNorm,
            weight: 0.3,
            contribution: demandNorm * 0.3,
            label: 'Demand Strength',
            factors: { feedbackCount: theme.feedbackCount ?? 0 },
          },
          revenueImpact: {
            raw: revenueNorm,
            normalised: revenueNorm,
            weight: 0.35,
            contribution: revenueNorm * 0.35,
            label: 'Revenue Impact',
            factors: { revenueInfluence: theme.revenueInfluence ?? 0 },
          },
          strategicImportance: {
            raw: strategicNorm,
            normalised: strategicNorm,
            weight: 0.2,
            contribution: strategicNorm * 0.2,
            label: 'Strategic Importance',
            factors: {
              tagMultiplier:
                theme.strategicTag === 'strategic'
    ...
    const items: RoadmapRecommendationItem[] = roadmapItems.map((item) => {
      ...
      const tagMult = theme?.strategicTag
        ? (STRATEGIC_TAG_MULTIPLIER[theme.strategicTag] ?? 1.0)
        : 1.0;
      const statusMult = ROADMAP_STATUS_MULTIPLIER[item.status] ?? 1.0;
      const feedbacks = theme?.feedbacks.map((f) => f.feedback) ?? [];
      const avgSentiment =
        feedbacks.length > 0
          ? feedbacks.reduce((s, f) => s + (f.sentiment ?? 0), 0) /
      ...
      const demandNorm = clamp100(
        countNorm(feedbacks.length, 30) * 0.4 +
          (avgSentiment < 0 ? Math.abs(avgSentiment) * 100 : 0) * 0.3,
      );
      const revenueNorm = clamp100(
        logNorm(totalArr, 8) * 0.4 +
          logNorm(dealInfluence, 7) * 0.4 +
          maxChurnRisk * 100 * 0.2,
      );
      const strategicNorm = clamp100(
        (theme?.strategicTag === 'strategic'
          ? 100
      ...
      const urgencyNorm = clamp100(
        avgUrgency * 100 * 0.6 +
          (maxChurnRisk >= 0.7 ? 80 : maxChurnRisk >= 0.4 ? 40 : 0) * 0.4,
      );
      ...
      if (item.status === RoadmapStatus.SHIPPED) {
        recommendation = 'already_shipped';
        rationale = 'Already shipped.';
      } else if (
        roadmapScore >= 75 &&
        item.status === RoadmapStatus.EXPLORING
      ) {
        recommendation = 'promote_to_committed';
        rationale = `High priority score (${roadmapScore.toFixed(0)}) — ready to commit.`;
      } else if (roadmapScore >= 55 && item.status === RoadmapStatus.BACKLOG) {
        recommendation = 'promote_to_planned';
        rationale = `Moderate-high priority score (${roadmapScore.toFixed(0)}) — recommend moving to Planned.`;
      } else if (roadmapScore < 25 && item.status !== RoadmapStatus.COMMITTED) {
        recommendation = 'deprioritise';
        rationale = `Low priority score (${roadmapScore.toFixed(0)}) — consider deprioritising.`;
      } else {
        recommendation = 'keep_current';
        rationale = `Priority score (${roadmapScore.toFixed(0)}) is appropriate for current status.`;
      }
      ...
      const breakdown: PrioritizationScoreBreakdown = {
        demandStrength: {
          ...
          contribution: demandNorm * weights.demandStrengthWeight,
          label: 'Demand Strength',
          factors: { feedbackCount: feedbacks.length, avgSentiment },
        },
        revenueImpact: {
          raw: revenueNorm,
          normalised: revenueNorm,
          ...
          contribution: revenueNorm * weights.revenueImpactWeight,
          label: 'Revenue Impact',
          factors: { totalArr, dealInfluence, maxChurnRisk },
        },
        strategicImportance: {
          raw: strategicNorm,
          normalised: strategicNorm,
        ...
        urgencySignal: {
          ...
          contribution: urgencyNorm * weights.urgencySignalWeight,
          label: 'Urgency Signals',
          factors: { avgUrgency, maxChurnRisk },
        },
      };
    ...
    return items
      .sort(
        (a, b) => b.roadmapRecommendationScore - a.roadmapRecommendationScore,
      )
      .slice(0, limit);
  }
  ...
  async getOpportunities(
    workspaceId: string,
    limit = 20,
  ): Promise<PrioritizationOpportunity[]> {
    ...
    for (const theme of themes) {
      ...
      if (
        roadmapStatus === RoadmapStatus.COMMITTED ||
        roadmapStatus === RoadmapStatus.SHIPPED
      )
        continue;
      const dealCount = theme.dealLinks.filter(
        (dl) => dl.deal?.status !== DealStatus.LOST,
      ).length;
      const arrAtRisk = theme.feedbacks.reduce(
        (s, f) =>
          s +
          ((f.feedback.customer?.churnRisk ?? 0) >= 0.5
            ? (f.feedback.customer?.arrValue ?? 0)
            : 0),
        0,
      );
      const opportunityScore = clamp100(
        (theme.revenueScore ?? 0) * 0.5 +
          (theme.urgencyScore ?? 0) * 0.3 +
          logNorm(arrAtRisk, 7) * 0.2,
      );
    ...
    for (const fb of feedbacks) {
      ...
      const arrAtRisk =
        (fb.customer?.churnRisk ?? 0) >= 0.5 ? (fb.customer?.arrValue ?? 0) : 0;
      const opportunityScore = clamp100(
        (fb.urgencySignal ?? 0) * 100 * 0.6 + logNorm(arrAtRisk, 7) * 0.4,
      );
  ...
  async persistThemeScores(
    ...
    this.logger.log(
      `Persisted priority scores for ${items.length} themes in workspace ${workspaceId}`,
    );
  }

  async persistFeedbackSignals(
    workspaceId: string,
    items: FeaturePriorityItem[],
  ): Promise<void> {
    await Promise.all(
      items.map((item) =>
        this.prisma.feedback.update({
          ...
          data: {
            ...
            urgencySignal: item.urgencyScore / 100,
            voteVelocity: item.voteVelocity,
          },
        }),
      ),
    );
    this.logger.log(
      `Persisted urgency signals for ${items.length} feedback items in workspace ${workspaceId}`,
    );
  }
  ...
  async getThemeData(workspaceId: string, themeId: string): Promise<ThemeData> {
    ...
    const uniqueCustomers = [
      ...new Set(feedback.map((f) => f.customerId).filter(Boolean)),
    ];
    return {
      themeId,
