...
export class DashboardAggregationService {
  ...
  async getProductDirection(workspaceId: string): Promise<ProductDirectionSummary> {
    const [feedbacks, totalCount] = await Promise.all([
      this.prisma.feedback.findMany({
        where: { workspaceId, status: { not: 'MERGED' }, ciqScore: { gt: 0 } },
        select: {
          id: true, title: true, ciqScore: true, urgencySignal: true,
          voteVelocity: true, sentiment: true, createdAt: true,
          customer: { select: { arrValue: true, accountPriority: true, churnRisk: true } },
          votes: { select: { id: true } },
          themes: { select: { theme: { select: { id: true, title: true, revenueInfluence: true } } }, take: 1 },
          mergedFrom: { select: { id: true } },
        },
        orderBy: { ciqScore: { sort: 'desc', nulls: 'last' } },
        take: 20,
      }),
      this.prisma.feedback.count({ where: { workspaceId, status: { not: 'MERGED' } } }),
    ]);
    ...
    const top3 = feedbacks.slice(0, 3).map((fb) => {
      ...
      const rationale = this.buildFeatureRationale(fb, voteCount, arrValue, ciqScore);
  ...
  private buildFeatureRationale(
    fb: { ciqScore: number | null; urgencySignal: number | null; sentiment: number | null; customer?: { churnRisk: number | null } | null },
    voteCount: number,
    arrValue: number,
    ciqScore: number,
  ): string {
    ...
    return parts.length > 0 ? `Driven by ${parts.join(', ')}.` : 'Emerging demand signal.';
  }
  ...
  async getEmergingThemes(workspaceId: string): Promise<EmergingThemeRadar> {
    ...
    const [themes, spikeEvents, totalActive] = await Promise.all([
      ...
      this.prisma.issueSpikeEvent.findMany({
        ...
        select: {
          id: true, ticketCount: true, zScore: true, windowStart: true,
    ...
    const emerging = themes
      .map((t) => {
        ...
        const recent7d    = allFeedback.filter((f) => f.createdAt >= sevenDaysAgo).length;
        const recent30d   = allFeedback.filter((f) => f.createdAt >= thirtyDaysAgo).length;
        const prev30d     = allFeedback.filter((f) => f.createdAt >= ninetyDaysAgo && f.createdAt < thirtyDaysAgo).length;
    ...
    return { emergingThemes: emerging, spikeEvents: spikes, totalActiveThemes: totalActive };
  }
  ...
  async getRevenueRisk(workspaceId: string): Promise<RevenueRiskIndicator> {
    const [atRiskCustomers, churnLinkedFeedback, segments] = await Promise.all([
      this.prisma.customer.findMany({
        ...
        select: {
          id: true, name: true, arrValue: true, churnRisk: true,
          accountPriority: true, segment: true,
          feedbacks: {
        ...
        orderBy: [{ churnRisk: 'desc' }, { arrValue: { sort: 'desc', nulls: 'last' } }],
        take: 20,
      }),
      this.prisma.feedback.findMany({
        where: { workspaceId, urgencySignal: { gte: 0.4 }, status: { not: 'MERGED' } },
        select: {
          id: true, title: true, urgencySignal: true,
          customer: { select: { arrValue: true, churnRisk: true, segment: true } },
    ...
    const totalArrAtRisk = atRiskCustomers.reduce((s, c) => s + (c.arrValue ?? 0), 0);
  ...
  async getVoiceSentiment(workspaceId: string): Promise<VoiceSentimentSignal> {
    ...
    const [recentFeedback, prevFeedback, themes, voiceCount] = await Promise.all([
      this.prisma.feedback.findMany({
        where: { workspaceId, sentiment: { not: null }, createdAt: { gte: thirtyDaysAgo } },
        select: {
          id: true, title: true, sentiment: true, urgencySignal: true, createdAt: true,
      ...
      this.prisma.feedback.findMany({
        where: { workspaceId, sentiment: { not: null }, createdAt: { gte: sixtyDaysAgo, lt: thirtyDaysAgo } },
    ...
    const avgRecent = recentFeedback.length > 0
      ? recentFeedback.reduce((s, f) => s + (f.sentiment ?? 0), 0) / recentFeedback.length
      : 0;
    const avgPrev = prevFeedback.length > 0
      ? prevFeedback.reduce((s, f) => s + (f.sentiment ?? 0), 0) / prevFeedback.length
      : 0;

    const sentimentTrend: VoiceSentimentSignal['sentimentTrend'] =
      avgRecent > avgPrev + 0.05 ? 'improving' :
      avgRecent < avgPrev - 0.05 ? 'declining' : 'stable';

    const negativeFraction = recentFeedback.length > 0
      ? recentFeedback.filter((f) => (f.sentiment ?? 0) < -0.2).length / recentFeedback.length
      : 0;

    const negativeTrend = negativeFraction > 0.3 || sentimentTrend === 'declining';

    // Build unresolved pain summary
    const topNegative = recentFeedback.filter((f) => (f.sentiment ?? 0) < -0.3).slice(0, 3);
    const unresolvedPain = topNegative.length > 0
    ...
    const sentimentByTheme = themes
      .map((t) => {
        ...
        const negFrac = sentiments.filter((s) => s < -0.2).length / sentiments.length;
        return {
          themeId:          t.id,
          title:            t.title,
  ...
  async getSupportPressure(workspaceId: string): Promise<SupportPressureIndicator> {
    const sevenDaysAgo  = new Date(Date.now() - 7  * 24 * 60 * 60 * 1000);
    const fourteenDaysAgo = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000);

    const [openTickets, recent7d, prev7d, clusters, spikeEvents] = await Promise.all([
      this.prisma.supportTicket.count({ where: { workspaceId, status: 'OPEN' } }),
      this.prisma.supportTicket.count({ where: { workspaceId, createdAt: { gte: sevenDaysAgo } } }),
      this.prisma.supportTicket.count({ where: { workspaceId, createdAt: { gte: fourteenDaysAgo, lt: sevenDaysAgo } } }),
      this.prisma.supportIssueCluster.findMany({
        where: { workspaceId },
        select: {
          id: true, title: true, ticketCount: true, arrExposure: true,
      ...
      this.prisma.issueSpikeEvent.count({ where: { workspaceId, windowStart: { gte: sevenDaysAgo } } }),
    ]);
  ...
  async getRoadmapHealth(workspaceId: string): Promise<RoadmapHealthPanel> {
    ...
    const [roadmapItems, highPriorityThemes] = await Promise.all([
      this.prisma.roadmapItem.findMany({
        ...
        select: {
          ...
          theme: { select: { id: true, title: true, priorityScore: true, revenueScore: true } },
          priorityScore: true,
        },
    ...
    const shippedRatio = total > 0 ? parseFloat((counts[RoadmapStatus.SHIPPED] / total).toFixed(2)) : 0;

    // Delayed critical items: PLANNED or COMMITTED items not updated in 30+ days with high priority
    const delayed = roadmapItems
      .filter((r) =>
        (r.status === RoadmapStatus.PLANNED || r.status === RoadmapStatus.COMMITTED) &&
      ...
      .map((r) => ({
        ...
        daysInStatus:  Math.floor((Date.now() - r.updatedAt.getTime()) / (1000 * 60 * 60 * 24)),
