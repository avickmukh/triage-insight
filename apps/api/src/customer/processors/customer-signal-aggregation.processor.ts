...
export class CustomerSignalAggregationProcessor {
  ...
  async handle(job: Job<CustomerSignalAggregationJobPayload>) {
    ...
    try {
      switch (type) {
        ...
        default:
          this.logger.warn(`Unknown CustomerSignalAggregation job type: ${type}`);
      }
    } catch (err) {
      this.logger.error(
  ...
  async aggregateCustomer(workspaceId: string, customerId: string): Promise<void> {
  ...
  async aggregateWorkspace(workspaceId: string): Promise<void> {
    ...
    this.logger.log(`Aggregating signals for ${customers.length} customers in workspace ${workspaceId}`);
  ...
  private computeScores(customer: {
    ...
    feedbacks: Array<{ sentiment?: number | null; impactScore?: number | null; createdAt: Date; submittedAt: Date | null }>;
    ...
    const arrTierMultiplier = arr >= 100_000 ? 1.5 : arr >= 50_000 ? 1.25 : arr >= 10_000 ? 1.1 : 1.0;
    ...
    const openDealValue = deals.filter((d) => d.status === 'OPEN').reduce((s, d) => s + d.annualValue, 0);
    ...
    const lastDate = allDates.length > 0 ? new Date(Math.max(...allDates.map((d) => d.getTime()))) : null;
    const daysSinceLast = lastDate ? (Date.now() - lastDate.getTime()) / (1000 * 60 * 60 * 24) : 365;
    const recencyScore = Math.max(0, 100 - daysSinceLast * 2); // -2 pts per day, floor 0

    const healthScore = Math.round(
      lifecycleScore * 0.4 +
      churnRiskInverse * 0.4 +
      recencyScore * 0.2,
    );
