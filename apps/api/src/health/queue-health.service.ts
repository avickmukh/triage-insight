  ...
  private async getQueueStats(name: string, queue: Queue): Promise<QueueStats> {
    try {
      ...
      if (waiting > this.WAIT_WARN_THRESHOLD) {
        warnings.push(`waiting depth ${waiting} exceeds threshold ${this.WAIT_WARN_THRESHOLD}`);
      }
      if (failed > this.FAIL_WARN_THRESHOLD) {
        warnings.push(`failed count ${failed} exceeds threshold ${this.FAIL_WARN_THRESHOLD}`);
      }
      if (isPaused) {
